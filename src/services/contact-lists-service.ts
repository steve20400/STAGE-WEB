...
/**
 * `null` tant que rien n'a ete lu : l'initialisation ne peut pas se faire a
 * l'import, ce module etant aussi charge la ou `window` n'existe pas.
 */
let miroir: ListeContacts[] | null = null

/** L'etat interne lui-meme, reserve a ce module : il ne sort jamais tel quel. */
function miroirCourant(): ListeContacts[] {
  if (!miroir) miroir = lireMiroir()
  return miroir
}
...
export async function creerListe(
  ...
  try {
    ...
    const liste = listeObligatoire(reponse.list)
