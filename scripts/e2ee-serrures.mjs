 ...
 *
 * Usage : node scripts/e2ee-serrures.mjs   (aucun serveur requis)
 */
...
const SORTIE = "scripts/.banc-serrures";

if (existsSync(SORTIE)) rmSync(SORTIE, { recursive: true, force: true });
console.log("\n⚙  Compilation du scénario par Vite…");
execSync(
  `npx vite build --ssr scripts/e2ee-serrures-test.js --outDir ${SORTIE} --logLevel error`,
  { stdio: "inherit" },
);

const module = await import(`../${SORTIE}/e2ee-serrures-test.js`);
